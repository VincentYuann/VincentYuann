...
import { ArrowLeft, Search, ExternalLink, Github, Filter, Layers, Waves } from 'lucide-react';
import { PROJECTS, Project } from '../data/projects';
import { ProjectDetailModal } from './ProjectDetailModal';
import { EnsoOrbital } from './EnsoOrbital';
import { HankoStamp } from './HankoStamp';
import { JapaneseOceanBackground } from './JapaneseOceanBackground';
...
export const ProjectsPage: React.FC<ProjectsPageProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="relative w-full min-h-screen">
      {/* 🌊 Japanese Ocean Theme Ambient Layer (The Great Wave & Seigaiha) */}
      <JapaneseOceanBackground />
      ...
      <div className="relative z-10 w-full pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        {/* Header Section */}
        <div className="mb-10 pb-6 border-b border-light-border/80 dark:border-dark-border/80 backdrop-blur-[2px]">
          <button
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <div className="flex items-center gap-2 mb-2">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-mono tracking-wider bg-[#1b3d68]/10 dark:bg-[#2b598f]/25 text-[#1b3d68] dark:text-[#88b9ea] border border-[#1b3d68]/20 dark:border-[#386ea8]/40">
                  <Waves className="w-3 h-3" />
                  <span>波濤 · THE GREAT WAVE ARCHIVE</span>
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-serif text-sm text-[#1b3d68] dark:text-[#88b9ea]">作品全集</span>
              </div>
            ...
            <div className="relative w-full md:w-72">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-light-surface-card/90 dark:bg-[#181920]/90 backdrop-blur-md border border-light-border dark:border-[#2D3039] rounded-lg text-xs font-sans text-light-ink dark:text-dark-ink focus:outline-none focus:border-terracotta transition-colors shadow-xs"
              />
            </div>
          </div>
        ...
        {filteredProjects.length === 0 ? (
          <div className="p-16 text-center rounded-xl bg-light-surface-card/95 dark:bg-[#181920]/95 backdrop-blur-md border border-light-border dark:border-[#2D3039]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProjects.map((project) => (
              <div
                key={project.id}
                className="interactive-card group relative bg-light-surface-card/95 dark:bg-[#181920]/95 backdrop-blur-md border border-light-border dark:border-[#2D3039] rounded-xl overflow-visible p-5 shadow-akari dark:shadow-night-glow hover:border-terracotta/40 transition-all duration-300 flex flex-col justify-between"
              >
